...
class ReqMeta:
    ...
    def from_request(request: "Request", 
        ...
        valid_num_tokens = align_to_block_size(
                len(request.prompt_token_ids), block_size)
...
class SharedStorageConnector(KVConnectorBase):
    ...
    def start_load_kv(
        ...
        def inject_kv_into_layer(
            ...
            Args:
                ...
                slot_mapping (torch.Tensor): the slot mapping. In shape 
                    [num_tokens].
            """
            num_tokens = src_kv_cache.shape[1]
        ...
        for layer_name in forward_context.no_compile_layers.keys():
            ...
            for request in metadata.requests:
                if request.is_store:
                    continue
                filename = self.generate_filename_debug(layer_name, request.token_ids)
                kv_cache = torch.load(filename).cuda() # TODO: may need to handle the device here
    ...
    def save_kv_layer(
        ...
        """
        # NOTE: kv_layer = _forward_context.no_compile_layers['model.layers.1.self_attn.attn'].kv_cache
        ...
        for request in connector_metadata.requests:
            if request.is_store:
                filename = self.generate_filename_debug(layer_name, request.token_ids)
                kv_cache = extract_kv_cache_from_layer(kv_layer, request.slot_mapping)
                torch.save(kv_cache.cpu().detach(), filename)
                logger.info(f"Saved KV cache of shape {kv_cache.shape} to {filename}")
    ...
    def get_external_prefix_cache_blocks(
        ...
        if need_to_allocate > 0:
            ...
            allocated_blocks = kv_cache_manager.allocate_slots(
                    request, need_to_allocate, computed_blocks, 
                    preallocate=False)
    ...
    def found_match_for_request(
        ...
        num_tokens_to_check = align_to_block_size(
                len(request.prompt_token_ids) - 1, self._block_size)
        foldername = self.generate_foldername_debug(
    ...
    def generate_foldername_debug(
        ...
        foldername = f"/tmp/{input_ids_hash}"
        if create_folder:
    ...
    def generate_filename_debug(
        ...
        """
        foldername = self.generate_foldername_debug(input_ids, create_folder=True)
