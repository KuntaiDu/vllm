...
main() {
    ...
    echo -e "\n\nNormal PD disaggregation test...\n\n" | tee $CURRENT_DIRECTORY/benchmark.log
    python benchmark_serving.py --port 9000 --seed 1 \
        --model meta-llama/Llama-3.1-8B-Instruct \
        --dataset-name random --random-input-len 5000 --random-output-len 200 \
        --num-prompts 5 --burstiness 100 --request-rate 1 | tee -a $CURRENT_DIRECTORY/benchmark.log

    echo -e "\n\nSending the requests to the prefiller only...\n\n" | tee -a $CURRENT_DIRECTORY/benchmark.log
    python benchmark_serving.py --port 8100 --seed 0 \
        --model meta-llama/Llama-3.1-8B-Instruct \
        --dataset-name random --random-input-len 5000 --random-output-len 200 \
        --num-prompts 5 --burstiness 100 --request-rate 1 | tee -a $CURRENT_DIRECTORY/benchmark.log

    echo -e "\n\nSending the requests to the decoder only...\n\n" | tee -a $CURRENT_DIRECTORY/benchmark.log
    python benchmark_serving.py --port 8200 --seed 0 \
        --model meta-llama/Llama-3.1-8B-Instruct \
        --dataset-name random --random-input-len 5000 --random-output-len 200 \
        --num-prompts 5 --burstiness 100 --request-rate 1 | tee -a $CURRENT_DIRECTORY/benchmark.log
